...
(function () {
  ...
  Repository.prototype = {
    ...
    checkSelection: function () {
      ...
      if (selectedElement && (widget = this.getByElement(selectedElement,
              true))) {
    ...
    del: function (widget) {
      if (this.focused === widget) {
        ...
        if (!(found = range.moveToClosestEditablePosition(widget.wrapper,
                true))) {
    ...
    getByElement: (function () {
      ...
      function getWidgetId(element) {
        return element.is(validWrapperElements) && element.data(
            'cke-widget-id');
      }
    ...
    wrapElement: function (element, widgetName) {
      ...
      if (element instanceof CKEDITOR.dom.element) {
        ...
        if (wrapper && wrapper.type == CKEDITOR.NODE_ELEMENT && wrapper.data(
                'cke-widget-wrapper')) {
          return wrapper;
        }
        ...
        if (!('data-cke-widget-keep-attr' in element.attributes)) {
          element.attributes['data-cke-widget-keep-attr'] = element.attributes['data-widget']
              ? 1 : 0;
        }
        if (widgetName) {
  ...
  function Widget(widgetsRepo, id, element, widgetDef, startupData) {
    ...
    CKEDITOR.tools.extend(this, widgetDef, {
      ...
      data: CKEDITOR.tools.extend({},
          typeof widgetDef.defaults == 'function' ? widgetDef.defaults()
              : widgetDef.defaults),
      ...
      _: {
        downcastFn: (widgetDef.downcast && typeof widgetDef.downcast
            == 'string') ? widgetDef.downcasts[widgetDef.downcast]
            : widgetDef.downcast
      }
    }, true);
    ...
    initEditable: function (editableName, definition) {
      ...
      if (editable && editable.is(CKEDITOR.dtd.$editable)) {
        editable = new NestedEditable(this.editor, editable, {
          filter: createEditableFilter.call(this.repository, this.name,
              editableName, definition)
        });
  ...
  Widget.isDomDragHandler = function (node) {
    return node.type == CKEDITOR.NODE_ELEMENT && node.hasAttribute(
        'data-cke-widget-drag-handler');
  };
  ...
  Widget.isDomDragHandlerContainer = function (node) {
    return node.type == CKEDITOR.NODE_ELEMENT && node.hasClass(
        'cke_widget_drag_handler_container');
  };
  ...
  function NestedEditable(editor, element, config) {
    ...
    if (!CKEDITOR.dtd[this.getName()].p) {
      ...
      this.enterMode = filter ? filter.getAllowedEnterMode(editor.enterMode)
          : editor.enterMode;
      this.shiftEnterMode = filter ? filter.getAllowedEnterMode(
          editor.shiftEnterMode, true) : editor.shiftEnterMode;
    }
  }
    ...
    editor.addCommand(widgetDef.name, {
      exec: function (editor, commandData) {
        ...
        // ... use insert method is was defined.
          ...
          var defaults = typeof widgetDef.defaults == 'function'
              ? widgetDef.defaults() : widgetDef.defaults,
          ...
          var editListener = instance.once('edit', function (evt) {
            if (evt.data.dialog) {
              instance.once('dialog', function (evt) {
                ...
                okListener = dialog.once('ok', finalizeCreation, null, null,
                    20);
  ...
  function addWidgetProcessors(widgetsRepo, widgetDef) {
    ...
    function addUpcast(upcast, name, priority) {
      ...
      var index = CKEDITOR.tools.getIndex(widgetsRepo._.upcasts,
          function (element) {
  ...
  function checkWidgets(evt) {
    ...
    if (options && options.initOnlyNew) {
      ...
      for (i = 0, count = wrappers.count(); i < count; i++) {
        ...
        if (notYetInitialized && !findParent(wrapper, isDomTemp)
  ...
  function createEditableFilter(widgetName, editableName, editableDefinition) {
    ...
    if (!filter) {
      filter = editableDefinition.allowedContent ? new CKEDITOR.filter(
          editableDefinition.allowedContent) : this.editor.filter.clone();
    ...
    return {
      ...
      iterator: function (element) {
        ...
        // and wrapped and stop iterating in this branch.
          ...
          for (i = 0, upcastCallbacksLength = upcastCallbacks.length;
              i < upcastCallbacksLength; ++i) {
  ...
  function isWidgetInline(widgetDef, elementName) {
    return typeof widgetDef.inline == 'boolean' ? widgetDef.inline
  ...
  function onEditableKey(widget, keyCode) {
    ...
    // DEL or BACKSPACE.
      ...
      return !(ranges.length == 1 && range.collapsed &&
          range.checkBoundaryOfElement(focusedEditable,
  ...
  function setFocusedEditable(widgetsRepo, widget, editableElement, offline) {
    ...
    if (editableElement) {
      ...
      editor.setActiveEnterMode(editableInstance.enterMode,
  ...
  function setupDragAndDrop(widgetsRepo) {
    ...
    editor.on('drop', function (evt) {
      ...
      evt.data.dataTransfer.setData('text/html',
          ...
          lookups: {
            ...
            'default': function (el) {
              ...
              if (nestedEditable) {
                ...
                var filter = CKEDITOR.filter.instances[nestedEditable.data(
                    'cke-filter')],
                ...
                if (filter && draggedRequiredContent && !filter.check(
  ...
  function setupMouseObserver(widgetsRepo) {
    ...
    editor.on('contentDom', function () {
      ...
      editable.attachListener(evtRoot, 'mousedown', function (evt) {
        ...
        widget = (target instanceof CKEDITOR.dom.element)
            ? widgetsRepo.getByElement(target) : null;
        ...
        if (widget) {
          ...
          if (widget.inline && target.type == CKEDITOR.NODE_ELEMENT
    ...
    editor.on('doubleclick', function (evt) {
      ...
      if (!widget || Widget.getNestedEditable(widget.wrapper,
              evt.data.element)) {
        return;
      }
  ...
  function setupSelectionObserver(widgetsRepo) {
    ...
    editor.on('selectionChange', function (evt) {
      var nestedEditable = Widget.getNestedEditable(editor.editable(),
          evt.data.selection.getStartElement()),
          newWidget = nestedEditable && widgetsRepo.getByElement(
              nestedEditable),
          oldWidget = widgetsRepo.widgetHoldingFocusedEditable;

      if (oldWidget) {
        if (oldWidget !== newWidget || !oldWidget.focusedEditable.equals(
                nestedEditable)) {
  ...
  function setupWidgetsLifecycle(widgetsRepo) {
    ...
    widgetsRepo.editor.on('contentDomInvalidated', widgetsRepo.checkWidgets,
        widgetsRepo);
  }
  ...
  function setupWidgetsLifecycleEnd(widgetsRepo) {
    ...
    editor.on('toDataFormat', function (evt) {
      ...
      evt.data.dataValue.forEach(function (element) {
        ...
        // Nested editable.
          ...
          toBeDowncasted[toBeDowncasted.length
          - 1].editables[attrs['data-cke-widget-editable']] = element;
    ...
    editor.on('toDataFormat', function (evt) {
      ...
      while ((toBe = toBeDowncasted.shift())) {
        ...
        retElement = widget._.downcastFn && widget._.downcastFn.call(widget,
            widgetElement);
  ...
  function setupWidgetsLifecycleStart(widgetsRepo) {
    ...
    editor.on('paste', function (evt) {
      ...
      data.dataValue = data.dataValue.replace(pasteReplaceRegex,
          pasteReplaceFn);
    ...
    editor.on('afterInsertHtml', function (evt) {
      if (evt.data.intoRange) {
        ...
        widgetsRepo.checkWidgets(
            {initOnlyNew: true, focusInited: processedWidgetOnly});
      ...
      commit: function () {
        ...
        while ((widget = toBeDeselected.pop())) {
          currentlySelected.splice(
              CKEDITOR.tools.indexOf(currentlySelected, widget), 1);
          // Widget could be destroyed in the meantime - e.g. data could be set.
          if (widget.isInited()) {
  ...
  function copySingleWidget(widget, isCut) {
    ...
    var listener1 = editor.on('selectionChange', cancel, null, null, 0),
        listener2 = widget.repository.on('checkSelection', cancel, null, null,
            0);
  ...
  function onEditableBlur() {
    ...
    if ((editable.isInline() ? editable
            : editor.document.getWindow().getFrame()).equals(active)) {
  ...
  function setupA11yListener(widget) {
    ...
    function getLabelDefault() {
      return this.editor.lang.widget.label.replace(/%1/, this.pathName
  ...
  function setupDragHandler(widget) {
    ...
    if (container) {
      ...
      container.setAttributes({
        ...
        + editor.plugins.widget.path + 'images/handle.png)'
      });
  ...
  function setupEditables(widget) {
    ...
    for (editableName in definedEditables) {
      ...
      widget.initEditable(editableName,
          typeof editableDef == 'string' ? {selector: editableDef}
              : editableDef);
    }
  }
  ...
  function setupWidget(widget, widgetDef) {
    ...
    if (CKEDITOR.env.ie && CKEDITOR.env.version < 9) {
      widget.wrapper.on('dragstart', function (evt) {
        ...
        if (!Widget.getNestedEditable(widget, target) && !(widget.inline
    ...
    widget.on('key', function (evt) {
      ...
      if (keyCode == 13) {
        widget.edit();
        // CTRL+C or CTRL+X.
      } else if (keyCode == CKEDITOR.CTRL + 67 || keyCode == CKEDITOR.CTRL
          + 88) {
        copySingleWidget(widget, keyCode == CKEDITOR.CTRL + 88);
        return; // Do not preventDefault.
      } else if (keyCode in keystrokesNotBlockedByWidget || (CKEDITOR.CTRL
              & keyCode) || (CKEDITOR.ALT & keyCode)) {
  ...
  function writeDataToElement(widget) {
    widget.element.data('cke-widget-data',
  ...
  (function () {
    ...
    CKEDITOR.style.addCustomHandler({
      ...
      setup: function (styleDefinition) {
         ...
         */
        this.group = typeof styleDefinition.group == 'string'
            ? [styleDefinition.group] : styleDefinition.group;
      ...
      getClassesArray: function () {
        var classes = this._.definition.attributes

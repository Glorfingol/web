...
import cmpl.web.page.TechnicsController;
...
public class TechnicsControllerTest {
  ...
  public void testPrintTechnicsTreatment() throws Exception {
    ...
    BDDMockito.doReturn(view).when(displayFactory)
        .computeModelAndViewForPage(Mockito.eq(PAGES.TECHNICS), Mockito.eq(locale));

    ModelAndView result = controller.printTechnicsTreatment();

    Assert.assertEquals(view, result);

    Mockito.verify(displayFactory, Mockito.times(1)).computeModelAndViewForPage(Mockito.eq(PAGES.TECHNICS),
        Mockito.eq(locale));
  }

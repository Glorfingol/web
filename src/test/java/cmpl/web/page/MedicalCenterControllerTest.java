...
import cmpl.web.page.MedicalCenterController;
...
public class MedicalCenterControllerTest {
  ...
  public void testPrintMedicalCenter() throws Exception {
    ...
    BDDMockito.doReturn(view).when(displayFactory)
        .computeModelAndViewForPage(Mockito.eq(PAGES.CENTER), Mockito.eq(locale));

    ModelAndView result = controller.printMedicalCenter();

    Assert.assertEquals(view, result);

    Mockito.verify(displayFactory, Mockito.times(1)).computeModelAndViewForPage(Mockito.eq(PAGES.CENTER),
        Mockito.eq(locale));
  }

...
import cmpl.web.page.OpeningHoursController;
...
public class OpeningHoursControllerTest {
  ...
  public void testPrintOpeningHours() throws Exception {
    ...
    BDDMockito.doReturn(view).when(displayFactory)
        .computeModelAndViewForPage(Mockito.eq(PAGES.OPENING_HOURS), Mockito.eq(locale));

    ModelAndView result = controller.printOpeningHours();

    Assert.assertEquals(view, result);

    Mockito.verify(displayFactory, Mockito.times(1)).computeModelAndViewForPage(Mockito.eq(PAGES.OPENING_HOURS),
        Mockito.eq(locale));
  }

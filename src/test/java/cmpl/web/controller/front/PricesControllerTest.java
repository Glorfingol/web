...
public class PricesControllerTest {
  ...
  public void testPrintPrices() throws Exception {
    ...
    BDDMockito.doReturn(view).when(displayFactory)
        .computeModelAndViewForPage(Mockito.eq(PAGE.PRICES), Mockito.eq(locale));

    ModelAndView result = controller.printPrices();

    Assert.assertEquals(view, result);

    Mockito.verify(displayFactory, Mockito.times(1)).computeModelAndViewForPage(Mockito.eq(PAGE.PRICES),
        Mockito.eq(locale));
  }
}

...
public class IndexControllerTest {
  ...
  public void testPrintIndex() throws Exception {
    ...
    BDDMockito.doReturn(view).when(displayFactory)
        .computeModelAndViewForPage(Mockito.eq(PAGE.INDEX), Mockito.eq(locale));

    ModelAndView result = controller.printIndex();

    Assert.assertEquals(view, result);

    Mockito.verify(displayFactory, Mockito.times(1)).computeModelAndViewForPage(Mockito.eq(PAGE.INDEX),
        Mockito.eq(locale));
  }
}

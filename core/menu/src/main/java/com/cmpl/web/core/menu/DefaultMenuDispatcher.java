...
public class DefaultMenuDispatcher implements MenuDispatcher {
  ...
  public DefaultMenuDispatcher(MenuTranslator translator, MenuService menuService,
  ...
  public MenuResponse createEntity(MenuCreateForm form, Locale locale) {
    ...
    if (!StringUtils.hasText(menuToCreate.getParentId()) && StringUtils
        .hasText(menuToCreate.getPageId())) {
  ...
  public MenuResponse updateEntity(MenuUpdateForm form, Locale locale) {
    ...
    MenuDTOBuilder menuToUpdateBuilder = MenuDTOBuilder.create().href(form.getHref())
        .label(form.getLabel())
        .orderInMenu(form.getOrderInMenu()).pageId(form.getPageId()).parentId(form.getParentId())
        .title(form.getTitle());

    if (!StringUtils.hasText(menuToUpdate.getParentId()) && StringUtils
        .hasText(menuToUpdate.getPageId())) {

...
public class DefaultMenuDispatcherTest {
  ...
  @Spy
  @InjectMocks
  private DefaultMenuDispatcher dispatcher;

  @Test
  public void testUpdateEntity_No_Error_ParentId() throws Exception {
    MenuUpdateForm form = MenuUpdateFormBuilder.create().id(123456789l).href("someHref")
        .label("someLabel")
    ...
    BDDMockito.given(menuService.updateEntity(BDDMockito.any(MenuDTO.class)))
        .willReturn(menuToUpdate);

    MenuResponse response = MenuResponseBuilder.create().menu(menuToUpdate).build();
    BDDMockito.given(translator.fromDTOToResponse(BDDMockito.any(MenuDTO.class)))
        .willReturn(response);
    ...
    BDDMockito.verify(translator, BDDMockito.times(1))
        .fromDTOToResponse(BDDMockito.any(MenuDTO.class));
  ...
  public void testCreateEntity_No_Error_ParentId() throws Exception {
    ...
    BDDMockito.given(menuService.createEntity(BDDMockito.any(MenuDTO.class)))
        .willReturn(menuToCreate);

    MenuResponse response = MenuResponseBuilder.create().menu(menuToCreate).build();
    BDDMockito.given(translator.fromDTOToResponse(BDDMockito.any(MenuDTO.class)))
        .willReturn(response);
    BDDMockito.given(translator.fromCreateFormToDTO(BDDMockito.any(MenuCreateForm.class)))
        .willReturn(menuToCreate);
    ...
    BDDMockito.verify(translator, BDDMockito.times(1))
        .fromDTOToResponse(BDDMockito.any(MenuDTO.class));
    BDDMockito.verify(translator, BDDMockito.times(1))
        .fromCreateFormToDTO(BDDMockito.any(MenuCreateForm.class));
    BDDMockito.verify(pageService, BDDMockito.times(0)).getEntity(BDDMockito.anyLong());

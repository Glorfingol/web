...
public class WebsiteTranslatorImpl implements WebsiteTranslator {
  ...
  public WebsiteDTO fromCreateFormToDTO(WebsiteCreateForm form) {
    return WebsiteDTOBuilder.create().description(form.getDescription()).name(form.getName()).build();
  }

  @Override
  public WebsiteDTO fromUpdateFormToDTO(WebsiteUpdateForm form) {
    return WebsiteDTOBuilder.create().description(form.getDescription()).name(form.getName()).id(form.getId())

...
public class DefaultWebsiteTranslator implements WebsiteTranslator {
  ...
  public WebsiteDTO fromUpdateFormToDTO(WebsiteUpdateForm form) {
    return WebsiteDTOBuilder.create().description(form.getDescription()).extension(form.getExtension())
        .secure(form.getSecure().booleanValue()).name(form.getName()).id(form.getId())
